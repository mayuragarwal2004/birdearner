...
import { Briefcase, CalendarBlank, CurrencyInr } from "phosphor-react-native";
...
const FreelancerJobHistory = ({ navigation }) => {
  ...
  const summary = useMemo(() => {
    ...
    const open = jobs.filter(
      (job) => !["COMPLETED", "CANCELLED", "REJECTED"].includes((job.jobStatus || "").toUpperCase())
    ).length;
    return { total, open };
  }, [jobs]);
  ...
  const fetchJobs = async () => {
    ...
    try {
      ...
      const response = await apiService.getJobsByFreelancerId(freelancerId, 1, 100);
      const fetchedJobs = response?.jobs || response?.data?.jobs || response || [];
      const nextJobs = Array.isArray(fetchedJobs) ? fetchedJobs : [];
      ...
      <FlatList
        ...
        ListHeaderComponent={
          <View style={styles.summaryRow}>
            ...
            <SummaryItem styles={styles} label="Active" value={summary.open} />
          </View>
        }
        ListEmptyComponent={
          loading ? (
            ...
            <View style={styles.stateContainer}>
              ...
              <Text style={styles.stateTitle}>No job history yet</Text>
...
function JobCard({ job, styles, isDark, onOptions }) {
  const status = getStatusMeta(job.jobStatus, isDark);
  const serviceName = job.serviceName || job.service?.name;
  ...
  return (
    <TouchableOpacity style={styles.jobCard} onPress={onOptions} activeOpacity={0.86}>
      <View style={styles.jobTopRow}>
        <View style={styles.serviceIcon}>
          {job.service?.imageUrl ? (
            <Image
              source={{ uri: apiService.loadImageURI(job.service.imageUrl) }}
      ...
      <View style={styles.jobBottomRow}>
        ...
        {!!job.clientName && (
          <Text style={styles.clientName} numberOfLines={1}>Client: {job.clientName}</Text>
        )}
      </View>
    </TouchableOpacity>
  );
...
function ActionRow({ styles, icon, label, onPress }) {
  return (
    <TouchableOpacity style={styles.actionRow} onPress={onPress}>
      {typeof icon === "string" ? (
        <Ionicons name={icon} size={23} color={PURPLE} />
      ) : (
        icon
      )}
...
export default FreelancerJobHistory;
