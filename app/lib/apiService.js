...
const DEV_API_BASE_URL = "https://ethical-available-internationally-punk.trycloudflare.com/api";
...
class ApiService {
  ...
  // Get reviews given by a user (where user is the reviewer)
  async getReviewsGivenByUserId(userId) {
    try {
      const response = await this.makeRequest(`/reviews/given/${userId}`);
      return response.data || response || [];
    } catch (error) {
      console.warn("Failed to fetch reviews given:", error.message);
      return [];
    }
  }

  // Get review statistics for reviews given by a user
  async getReviewStatsGiven(userId) {
    try {
      const response = await this.makeRequest(`/reviews/given-stats/${userId}`);
      return response.data || response || null;
    } catch (error) {
      console.warn("Failed to fetch review stats given:", error.message);
      return null;
    }
  }
